...
const EditForm = ({operation}) => {

	//Extract values from operation
	const {id, concept, amount, category, date} = operation

	//Extract values from context
	const AppContext = useContext(appContext)
	const {editOperation, loading} = AppContext
	...
	return(
		<form onSubmit={form.handleSubmit} className="p-5 pb-10 flex flex-col text-cyan-800 bg-white shadow rounded w-full">
			...
			<h2 className="text-cyan-800 text-center font-bold text-2xl py-2"><i className="fa-solid fa-edit"></i> Editar Operación</h2>
...

			{
				...
				: (<button type="submit" className="btn mt-3"><i className="fa-solid fa-save"></i> Guardar Cambios</button>)
			}
